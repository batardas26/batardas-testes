...
serve(async (req) => {
  ...
  if (acao === "listar_utilizadores") {
    const { data, error } = await admin.from("perfis")
      .select("id,nome,role,super_admin,nome_utilizador,email_contacto,perfil_base_codigo,estado_utilizador_a53,perfil_completo,deve_alterar_password").order("nome");
  ...
  if (acao === "listar_configuracao_utilizadores") {
    const [{ data: perfisBase, error: erroPerfis }, { data: capacidades, error: erroCapacidades }] = await Promise.all([
    ...
    if (erroPerfis || erroCapacidades) return resposta(req, { error: "Não foi possível carregar a configuração" }, 400);
    return resposta(req, { perfisBase, capacidades });
  }
