...
function aplicarModoOperacionalR31(perfil, area) {
    ...
    main.querySelectorAll("button:not(.nav-item), input, select, textarea").forEach(el=>{
        if(el.closest("[data-permitir-consulta-r31]"))return;
        el.disabled=true;
    });
    ...
    if(!document.body.dataset.bloqueioOperacionalR31){
        ...
        document.addEventListener("click",evento=>{
            ...
            if(!botao)return;evento.preventDefault();evento.stopImmediatePropagation();toast("Esta vista está em modo acompanhamento/consulta.","erro");
        },true);
    }
}
