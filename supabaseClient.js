...
function instalarBarraModuloR3() {
    ...
    abertos=[...new Set([...abertos.filter(x=>NOMES_MODULOS_R3[x]),pagina])];
    ...
    barra.innerHTML=abertos.map(x=>`<a href="${x}" class="${x===pagina?"activo":""}">${escaparHtml(NOMES_MODULOS_R3[x])}</a>`).join("")+`<button class="fechar-modulo" aria-label="Fechar módulo" title="Fechar módulo">×</button>`;
    barra.querySelector(".fechar-modulo").addEventListener("click",()=>{
        sessionStorage.setItem("batardas_modulos_abertos",JSON.stringify(abertos.filter(x=>x!==pagina)));
