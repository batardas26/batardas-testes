...
const VERSAO_FRONTEND = "R3.3-A.4.4.2";
...
function mostrarBackendDesatualizadoR323(estado = null) {
    ...
    const versaoBackend = estado?.versao ? escaparHtml(String(estado.versao)) : "não disponível";
    aviso.innerHTML = `<strong>Atualização incompleta</strong><p>A aplicação ${VERSAO_FRONTEND} encontrou a versão ${versaoBackend} na base de dados. Pede ao Super Admin para executar a migration 41 de unificação do planeamento. Nenhuma operação foi realizada.</p>`;
...
async function verificarBackendR323() {
    ...
    const valido = !error && data?.versao === VERSAO_FRONTEND
        ...
        && data?.conversoes_pa_formatos === true
        && data?.saldo_pa_recebido_r3341 === true
        && data?.bom_canonica_r3342 === true
        && data?.integridade_bom_r3343 === true
        && data?.fila_pa_entrega_r33432 === true
        && data?.elegibilidade_carga_r3344 === true
        && data?.cancelamento_palete_vazia_r33441 === true
        && data?.decomposicao_embalamento_unica_r33442 === true
        && data?.fecho_pa_robusto_r33431 === true
        && data?.continuidade_pa_r33432 === true;
    if (!valido) {
        estadoBackendR323 = false;
        mostrarBackendDesatualizadoR323(data);
        console.error("Backend incompatível com o frontend", { versaoFrontend: VERSAO_FRONTEND, versaoBackend: data?.versao ?? null, erro: error?.message ?? null });
        return false;
    }
...
function traduzirErro(mensagem) {
    ...
    const errosR31={
        ...
        PALETE_COM_CAIXAS_NAO_PODE_SER_CANCELADA:"Retira primeiro todas as caixas antes de cancelar a palete.",
        PALETE_NAO_ENCONTRADA:"A palete deixou de estar disponível. Atualiza a lista.",
        ...
        CODIGO_PALETE_OBRIGATORIO:"É obrigatório indicar o código da palete.",
        IDENTIFICADOR_OPERACAO_OBRIGATORIO:"Não foi possível identificar esta operação. Atualiza a página e tenta novamente.",
        ...
        FOTOGRAFIA_BOM_INVALIDA:"A produção não tem uma BOM válida associada.",
        COMPONENTE_OBRIGATORIO_BOM_EM_FALTA:"Seleciona pelo menos um lote de cada componente obrigatório da BOM.",
        MOTIVO_DESVIO_BOM_OBRIGATORIO:"Explica a diferença entre os consumos reais e a BOM prevista.",
...
async function obterTodosArtigos() {
    ...
    const { data, error } = await supabaseClient
        .from("artigos")
        .select("artigo_id, ref_primavera, designacao, unidade_stock, unidades_produto_por_stock")
        .order("designacao");
...
async function marcarNcPorConfirmarPartilhado(ncId) {
    ...
    const accao = await pedirTextoAcao({titulo:"Ação corretiva",mensagem:"Regista o que foi feito antes de enviar para confirmação da Qualidade.",etiqueta:"Ação realizada",confirmar:"Enviar para confirmação",valorInicial:actual?.accao_correctiva??""});
...
/** Confirmação consistente da aplicação; devolve true apenas no botão principal. */
function confirmarAcao({ titulo = "Confirmar", mensagem, confirmar = "Confirmar", perigo = false } = {}) {
    return new Promise((resolve) => {
        const fundo = document.createElement("div");
        fundo.className = "modal-r31";
        fundo.setAttribute("role", "dialog");
        fundo.setAttribute("aria-modal", "true");
        fundo.innerHTML = `<div class="cartao modal-confirmacao-r344"><h2></h2><p></p><div class="acoes"><button class="secundario" data-cancelar>Voltar</button><button data-confirmar></button></div></div>`;
        fundo.querySelector("h2").textContent = titulo;
        fundo.querySelector("p").textContent = mensagem ?? "";
        const botaoConfirmar = fundo.querySelector("[data-confirmar]");
        botaoConfirmar.textContent = confirmar;
        if (perigo) botaoConfirmar.classList.add("perigo");
        const fechar = (resultado) => {
            document.removeEventListener("keydown", aoTeclado);
            fundo.remove();
            resolve(resultado);
        };
        const aoTeclado = (evento) => { if (evento.key === "Escape") fechar(false); };
        fundo.querySelector("[data-cancelar]").addEventListener("click", () => fechar(false));
        botaoConfirmar.addEventListener("click", () => fechar(true));
        fundo.addEventListener("click", (evento) => { if (evento.target === fundo) fechar(false); });
...
/** Pedido de texto persistente para operações auditadas; null significa desistência. */
function pedirTextoAcao({ titulo, mensagem, etiqueta, confirmar = "Continuar", valorInicial = "", obrigatorio = true } = {}) {
    return new Promise((resolve) => {
        const fundo = document.createElement("div");
        fundo.className = "modal-r31";
        fundo.setAttribute("role", "dialog");
        fundo.setAttribute("aria-modal", "true");
        fundo.innerHTML = `<div class="cartao modal-confirmacao-r344"><h2></h2><p></p><label></label><textarea rows="3"></textarea><p class="erro"></p><div class="acoes"><button class="secundario" data-cancelar>Voltar</button><button data-confirmar></button></div></div>`;
        fundo.querySelector("h2").textContent = titulo ?? "Indicar motivo";
        fundo.querySelector("p").textContent = mensagem ?? "";
        fundo.querySelector("label").textContent = etiqueta ?? "Motivo";
        const campo = fundo.querySelector("textarea");
        campo.value = valorInicial ?? "";
        const erro = fundo.querySelector(".erro");
        fundo.querySelector("[data-confirmar]").textContent = confirmar;
        const fechar = (resultado) => {
            document.removeEventListener("keydown", aoTeclado);
            fundo.remove();
            resolve(resultado);
        };
        const aoTeclado = (evento) => { if (evento.key === "Escape") fechar(null); };
        fundo.querySelector("[data-cancelar]").addEventListener("click", () => fechar(null));
        fundo.querySelector("[data-confirmar]").addEventListener("click", () => {
            const valor = campo.value.trim();
            if (obrigatorio && !valor) { erro.textContent = "Preenche este campo."; campo.focus(); return; }
            fechar(valor);
        });
