...
const VERSAO_FRONTEND = "R3.3-A.4.3.2";
...
function mostrarBackendDesatualizadoR323() {
    ...
    aviso.innerHTML = `<strong>Atualização incompleta</strong><p>A aplicação ${VERSAO_FRONTEND} não encontrou a mesma versão na base de dados. Pede ao Super Admin para executar a migration 35 de integridade da BOM. Nenhuma operação foi realizada.</p>`;
...
async function verificarBackendR323() {
    ...
    const valido = !error && data?.versao === VERSAO_FRONTEND
        ...
        && data?.fila_pa_entrega_r3343 === true;
    if (!valido) {
