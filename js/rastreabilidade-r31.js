...
function linhasRastreioR31(dados) {
    ...
    (dados.comerciais||[]).forEach(x=>linhas.push(["Comercial",x.artigo,x.lote,`Inicial: ${x.quantidade_inicial}; físico: ${x.disponivel}; reservado: ${x.reservado}; livre: ${x.livre} ${x.unidade}`,x.validade||"—"]));
    (dados.logistica||[]).forEach(x=>linhas.push(["Logística",x.tipo,x.codigo,`${x.conteudo} ${x.unidade} · ${x.estado}`,"—"]));
    (dados.encomendas||[]).forEach(x=>linhas.push(["Cliente",x.cliente||"Sem cliente",x.lote_comercial||x.referencia||x.encomenda_id,`Encomenda ${x.encomenda_id}${x.quantidade!=null?` · ${x.quantidade}`:""} · ${x.estado}`,x.entrega||"—"]));
    return linhas;
}
...
async function pesquisarRastreioR31(numero,destino) {
    ...
    const {data,error}=await supabaseClient.rpc("rastreabilidade_total_r32",{p_pesquisa:valor});
    if(error){destino.innerHTML=`<p class="erro">${escaparHtml(traduzirErro(error.message))}</p>`;return null;}
    if(data?.contrato!=="R3.2.3"){
        destino.innerHTML='<p class="erro">A rastreabilidade da base de dados está desatualizada. Nenhum resultado foi assumido como válido.</p>';
        return null;
    }
