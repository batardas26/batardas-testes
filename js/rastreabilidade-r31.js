...
async function pesquisarRastreioR31(numero,destino) {
    ...
    let {data,error}=await supabaseClient.rpc("rastreabilidade_total_r32",{p_pesquisa:valor});
    if(error&&/function|schema cache|rastreabilidade_total/i.test(error.message||"")){
        const anterior=await supabaseClient.rpc("obter_rastreabilidade_r31",{p_numero:valor});
        data=anterior.data;error=anterior.error;
    }
