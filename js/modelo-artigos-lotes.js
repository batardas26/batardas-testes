...
document.getElementById("btnGuardarConversaoNova").addEventListener("click", async () => {
    ...
    const { error } = await supabaseClient.rpc("configurar_conversao_artigo_r334", { p_artigo: artigoId, p_unidade: registo.unidade_origem, p_fator_para_stock: registo.fator_para_stock, p_uso: registo.uso });
    if (error) { erroModelo("erroConversaoNova", traduzirErro(error.message)); return; }
    toast("Conversão guardada; a direção inversa é calculada automaticamente.", "sucesso"); carregarConversoesArtigo(artigoId);
});
...
async function carregarLotesModelo() {
    ...
    const [{ data: lotes, error }, todosArtigos, { data: locais }] = await Promise.all([
        supabaseClient.from("lotes_artigo")
            .select("lote_artigo_id,artigo_id,numero_lote,quantidade_atual,validade,validade_hora,apresentacao,estado_conservacao,estado_qualidade,localizacao_id,criado_em")
            .gt("quantidade_atual", 0).eq("estado_qualidade", "disponivel").order("criado_em", { ascending: true }),
        obterTodosArtigos(),
        supabaseClient.from("localizacoes").select("localizacao_id,nome"),
    ]);
    if (error) {
        console.error(error);
        erroModelo("erroTransformar", `Não foi possível carregar os lotes: ${traduzirErro(error.message)}`);
        return;
    }
    const artigosDetalhe = await supabaseClient.from("artigos")
        .select("artigo_id,designacao,unidade_stock,configuracao_unidades_confirmada,familia_validade_id")
        .in("artigo_id", [...new Set((lotes ?? []).map((l) => l.artigo_id))]);
    const mapaArtigos = new Map((artigosDetalhe.data ?? []).map((a) => [a.artigo_id, a]));
    const mapaLocais = new Map((locais ?? []).map((l) => [l.localizacao_id, l]));
    lotesModelo = (lotes ?? [])
        .filter((l) => !l.validade || l.validade >= hoje)
        .filter((l) => !l.validade_hora || new Date(l.validade_hora).getTime() >= Date.now())
        .map((l) => ({ ...l, artigos: mapaArtigos.get(l.artigo_id) ?? null, localizacoes: mapaLocais.get(l.localizacao_id) ?? null }));
...
function preencherLotesDoArtigo(idArtigo,idLote) {
    ...
    select.innerHTML=!artigoId?`<option value="">2. Escolhe primeiro o artigo</option>`:`<option value="">${elegiveis.length ? "2. Escolhe o lote..." : "Sem lotes válidos e disponíveis"}</option>`+elegiveis.map(l=>`<option value="${l.lote_artigo_id}">${escaparHtml(l.numero_lote)} · ${l.quantidade_atual} ${unidadeVisivel(l.artigos?.unidade_stock)} · validade ${formatarData(l.validade)} · ${escaparHtml(l.localizacoes?.nome??"sem armazém")}</option>`).join("");
}
